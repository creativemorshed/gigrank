    ...
    <div style={{minHeight:"100vh",background:th.bg,transition:"background 0.3s",fontFamily:"'DM Sans',system-ui,sans-serif",color:th.text}}>
      <style>{`*{box-sizing:border-box;margin:0;padding:0} input::placeholder,textarea::placeholder{color:${th.border}} @keyframes spin{to{transform:rotate(360deg)}} @keyframes fadeUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}
        ...
        @media(max-width:480px){
          ...
          .tagline-text{font-size:8px!important;letter-spacing:1px!important}
        }
      `}</style>
        ...
        {!mod&&(
          <div className="home-hero" style={{paddingTop:44,paddingBottom:36,animation:"fadeUp 0.5s ease"}}>
            <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:18}}>
              <div style={{width:8,height:8,borderRadius:"50%",background:"#00d4aa",boxShadow:"0 0 12px #00d4aa",flexShrink:0,marginTop:3}}/>
              <span className="tagline-text" style={{fontFamily:"'Syne',sans-serif",fontSize:9,fontWeight:700,letterSpacing:2,color:th.muted,lineHeight:1.5}}>{t.tagline}</span>
            </div>
            <h1 className="home-h1" style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(28px,8vw,56px)",fontWeight:900,lineHeight:1.05,letterSpacing:-1,marginBottom:14,color:th.text}}>
              {t.h1a}<br/>
              <span style={{WebkitTextStroke:`1px ${th.border}`,color:"transparent",display:"inline-block",maxWidth:"100%"}}>{t.h1b}</span>
            </h1>
            <p style={{color:th.muted,fontSize:14,lineHeight:1.7,maxWidth:520,fontWeight:300,marginBottom:24}}>{t.desc}</p>
