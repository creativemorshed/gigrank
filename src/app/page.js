...
import { supabase, getCreditsFromDB, deductCredits } from "@/lib/supabase";
...
const T = {
    ...
    h1a:"GIG", h1b:"RANKING",
    desc:"Analyze gigs, spy on competitors, predict rankings, rewrite content, and generate 90-day growth roadmaps — Powered by Fiverr's 2026 Algorithm.",
    ...
    algoBreakdown:"RANKING FACTORS BREAKDOWN 2026",
...
function PredictResult({r,t,th}){
  ...
  return(
    <div>
      <div className="three-col" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:12}}>
        {days.map(({l,d,c})=>(
...
//  PRICING MODAL
// ─────────────────────────────────────────
function PricingModal({onClose,onAdded,t,th,userId}){
  ...
  const redeem=async()=>{
    ...
    try{
      const res=await fetch("/api/redeem",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:k,userId:userId})});
  ...
  return(
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(3,7,18,0.92)",backdropFilter:"blur(12px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:th.surface,border:`1px solid ${th.border}`,borderRadius:24,padding:32,maxWidth:620,width:"100%",maxHeight:"90vh",overflowY:"auto",position:"relative"}}>
        ...
        <div className="three-col" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:24}}>
          {PLANS.map(p=>(
...
// ─────────────────────────────────────────
// ─────────────────────────────────────────
//  ALGO FACTORS — COLLAPSIBLE, 2-COLUMN
// ─────────────────────────────────────────
function AlgoFactorsCollapse({ th, t }) {
  const [open, setOpen] = useState(false);
  return (
    <div style={{ marginBottom: 32 }}>
      {/* Toggle button — full width, no text wrap */}
      <button
        onClick={() => setOpen(!open)}
        style={{
          display: "flex", alignItems: "center", gap: 8,
          background: th.surface,
          border: `1px solid ${open ? "#00d4aa" : th.border}`,
          borderRadius: 12, padding: "11px 14px", cursor: "pointer",
          transition: "all 0.2s", width: "100%",
          boxShadow: open ? "0 0 0 3px #00d4aa11" : "none",
          overflow: "hidden",
        }}
      >
        ...
        <span style={{
          ...
          color: open ? "#00d4aa" : th.text,
          flex: 1, textAlign: "left", overflow: "hidden",
          textOverflow: "ellipsis", whiteSpace: "nowrap",
        }}>
          Ranking Factors Breakdown 2026
        </span>
        <span style={{
          fontFamily: "'Syne',sans-serif", fontSize: 9, fontWeight: 700,
          color: "#00d4aa", background: "#00d4aa18", padding: "3px 7px",
          borderRadius: 20, border: "1px solid #00d4aa33", flexShrink: 0,
          whiteSpace: "nowrap",
        }}>11 Factors</span>
        <span style={{
          fontSize: 11, color: open ? "#00d4aa" : th.muted,
          transform: open ? "rotate(180deg)" : "none",
          transition: "transform 0.25s", flexShrink: 0, marginLeft: 2,
        }}>▾</span>
      </button>
      ...
      {open && (
        <div style={{
          ...
          borderRadius: 14, padding: "16px", animation: "fadeUp 0.25s ease",
        }}>
          ...
          <div className="algo-grid">
            {FACTORS.map(f => (
              <div key={f.key} style={{
                ...
                <span style={{ width: 8, height: 8, borderRadius: "50%", background: f.color, flexShrink: 0, boxShadow: `0 0 6px ${f.color}88` }}/>
                <span style={{
                  fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 11,
                  color: th.text, flex: 1, minWidth: 0, overflow: "hidden",
                  textOverflow: "ellipsis", whiteSpace: "nowrap",
                }}>
                  {f.label}
                </span>
                {f.isNew && (
                  <span style={{
                    fontSize: 8, fontWeight: 800, background: "#00d4aa18",
                    color: "#00d4aa", padding: "2px 5px", borderRadius: 4,
                    flexShrink: 0, letterSpacing: 0.5, border: "1px solid #00d4aa33",
                  }}>NEW</span>
                )}
                <span style={{
                  fontFamily: "'Syne',sans-serif", fontSize: 13, fontWeight: 900,
                  color: f.color, flexShrink: 0, minWidth: 32, textAlign: "right",
                }}>{f.w}%</span>
              </div>
            ))}
          </div>
          <div style={{
            fontSize: 10, color: th.muted, marginTop: 12, textAlign: "center",
            fontFamily: "'Syne',sans-serif", letterSpacing: 1,
          }}>
            BASED ON FIVERR'S OFFICIAL 2026 RANKING ALGORITHM
          </div>
        </div>
      )}
    </div>
...
function AuthModal({ onClose, onAuth, th }) {
  ...
  const inputStyle = {
    width: "100%", background: th.input, border: `1px solid ${th.border}`,
  ...
  const handleAuth = async () => {
    if (!email || !password) { setMsg({ text: "Please fill all fields.", ok: false }); return; }
    ...
    try {
      ...
      setMsg({ text: err.message || "Authentication failed.", ok: false });
    } finally { setLoading(false); }
  };

  const handleForgot = async () => {
    if (!email) { setMsg({ text: "Enter your email first.", ok: false }); return; }
    setLoading(true);
  ...
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()}
      style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(3,7,18,0.95)", backdropFilter: "blur(16px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: th.surface, border: `1px solid ${th.border}`, borderRadius: 24, padding: 36, maxWidth: 420, width: "100%", position: "relative" }}>
        <button onClick={onClose} style={{ position: "absolute", top: 16, right: 20, background: "none", border: "none", color: th.muted, fontSize: 20, cursor: "pointer" }}>✕</button>

        {/* Logo */}
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 900, fontSize: 22, color: th.text }}>
            Gig<span style={{ color: "#00d4aa" }}>Ranking</span>
          </div>
        ...
        {/* Google login */}
        {mode !== "forgot" && (
          <button onClick={handleGoogle} style={{ width: "100%", padding: "12px", border: `1px solid ${th.border}`, borderRadius: 10, background: "transparent", color: th.text, fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 13, cursor: "pointer", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
            <svg width="18" height="18" viewBox="0 0 48 48"><path fill="#FFC107" d="M43.6 20H24v8h11.3C33.7 33.3 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.1 7.9 2.9l5.7-5.7C34.1 6.5 29.3 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20c11 0 19.7-8 19.7-20 0-1.3-.1-2.7-.1-4z"/><path fill="#FF3D00" d="M6.3 14.7l6.6 4.8C14.5 15.1 18.9 12 24 12c3.1 0 5.8 1.1 7.9 2.9l5.7-5.7C34.1 6.5 29.3 4 24 4 16.3 4 9.6 8.3 6.3 14.7z"/><path fill="#4CAF50" d="M24 44c5.2 0 9.9-1.9 13.4-4.9l-6.2-5.2C29.4 35.6 26.8 36 24 36c-5.2 0-9.7-2.7-11.3-7L6 33.5C9.5 39.6 16.3 44 24 44z"/><path fill="#1976D2" d="M43.6 20H24v8h11.3c-.8 2.3-2.3 4.2-4.2 5.6l6.2 5.2C41 35.5 44 30.2 44 24c0-1.3-.1-2.7-.4-4z"/></svg>
        ...
        {mode !== "forgot" && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 11, color: th.muted }}>or</span>
            <div style={{ flex: 1, height: 1, background: th.border }} />
          </div>
        )}
        ...
        {mode !== "forgot" && (
          <input style={inputStyle} type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Password (min 6 chars)" onKeyDown={e => e.key === "Enter" && handleAuth()} />
        )}

        {msg.text && <div style={{ fontSize: 12, color: msg.ok ? "#00d4aa" : "#ef4444", marginBottom: 12 }}>{msg.text}</div>}

        {/* Main button */}
        <button onClick={mode === "forgot" ? handleForgot : handleAuth} disabled={loading}
          style={{ width: "100%", padding: 14, border: "none", borderRadius: 10, background: "linear-gradient(135deg,#00d4aa,#0ea5e9)", color: "#030712", fontFamily: "'Syne',sans-serif", fontWeight: 900, fontSize: 14, letterSpacing: 1, cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1, marginBottom: 16 }}>
          {loading ? "Please wait..." : mode === "login" ? "SIGN IN" : mode === "signup" ? "CREATE ACCOUNT" : "SEND RESET EMAIL"}
        </button>

        {/* Mode switchers */}
        <div style={{ textAlign: "center", fontSize: 12, color: th.muted }}>
          {mode === "login" && (<>No account? <button onClick={() => { setMode("signup"); setMsg({ text: "", ok: false }); }} style={{ background: "none", border: "none", color: "#00d4aa", cursor: "pointer", fontWeight: 700 }}>Sign up</button>{" · "}<button onClick={() => { setMode("forgot"); setMsg({ text: "", ok: false }); }} style={{ background: "none", border: "none", color: th.muted, cursor: "pointer" }}>Forgot password?</button></>)}
          {mode === "signup" && (<>Have account? <button onClick={() => { setMode("login"); setMsg({ text: "", ok: false }); }} style={{ background: "none", border: "none", color: "#00d4aa", cursor: "pointer", fontWeight: 700 }}>Sign in</button></>)}
          {mode === "forgot" && (<><button onClick={() => { setMode("login"); setMsg({ text: "", ok: false }); }} style={{ background: "none", border: "none", color: "#00d4aa", cursor: "pointer", fontWeight: 700 }}>← Back to Sign in</button></>)}
        </div>
      </div>
    </div>
  );
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    supabase.auth.getSession().then(async({ data:{ session } })=>{
      if(session?.user){
        ...
        const c = await getCreditsFromDB(session.user.id);
        setCredits(c);
      }
  ...
  const updC=async()=>{
    if(user){
      const c=await getCreditsFromDB(user.id);
      setCredits(c);
    }
  };
  ...
  const signOut=async()=>{
    ...
    setUser(null); setCredits(0); setMod(null); setResult(null);
  };
  ...
  const selectMod=m=>{
    ...
    if(credits<m.cost){ setModal(true); return; }
  ...
  const submit=useCallback(async payload=>{
    if(!mod) return;
    if(!user){ setShowAuth(true); return; }
    if(credits<mod.cost){ setModal(true); return; }
    ...
    try{
      ...
      // Deduct from Supabase DB
      await deductCredits(user.id, mod.cost);
      await updC();
  ...
  return(
    <div style={{minHeight:"100vh",background:th.bg,transition:"background 0.3s",fontFamily:"'DM Sans',system-ui,sans-serif",color:th.text}}>
      <style>{`*{box-sizing:border-box;margin:0;padding:0} input::placeholder,textarea::placeholder{color:${th.border}} @keyframes spin{to{transform:rotate(360deg)}} @keyframes fadeUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}
        ...
        @media(max-width:600px){
          ...
          .three-col{grid-template-columns:1fr!important}
          .home-hero{padding-left:4px;padding-right:4px}
        }
        .hero-grid{display:grid;grid-template-columns:1fr 1fr;gap:32px;margin-bottom:32px;align-items:center}
        .tagline-text{font-size:12px;letter-spacing:3px}
        @media(max-width:700px){
          .hero-grid{grid-template-columns:1fr!important;gap:24px!important}
          .hero-stats{display:grid!important;grid-template-columns:1fr 1fr;gap:8px}
        }
        @media(max-width:480px){
          .hero-stats{grid-template-columns:1fr!important}
          .tagline-text{font-size:10px!important;letter-spacing:2px!important}
      ...
      {/* NAV */}
      <div style={{position:"sticky",top:0,zIndex:100,background:th.nav,backdropFilter:"blur(16px)",borderBottom:`1px solid ${th.border}`,padding:"12px 20px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",maxWidth:900,margin:"0 auto"}}>
          <div onClick={reset} style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:16,letterSpacing:-0.5,color:th.text,cursor:"pointer",flexShrink:0}}>
            Gig<span style={{color:"#00d4aa"}}>Ranking</span>
          </div>
          ...
          <div className="nav-desktop" style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            {user?(
              <>
                ...
                <div style={{display:"flex",alignItems:"center",gap:6,background:th.surface,border:`1px solid ${th.border}`,borderRadius:20,padding:"5px 12px"}}>
                  <div style={{width:22,height:22,borderRadius:"50%",background:"linear-gradient(135deg,#00d4aa,#0ea5e9)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:900,color:"#030712",flexShrink:0}}>
                    {(user.email||"U")[0].toUpperCase()}
                  </div>
                  <span style={{fontSize:11,color:th.muted,maxWidth:90,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user.email?.split("@")[0]}</span>
                  <button onClick={signOut} style={{background:"none",border:"none",color:"#ef4444",fontSize:10,cursor:"pointer",fontFamily:"'Syne',sans-serif",fontWeight:700,padding:0,marginLeft:4}}>OUT</button>
                </div>
              </>
            ):(
              <button onClick={()=>setShowAuth(true)} style={{background:"linear-gradient(135deg,#00d4aa,#0ea5e9)",border:"none",borderRadius:20,padding:"8px 20px",color:"#030712",fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:12,letterSpacing:1,cursor:"pointer"}}>
                LOGIN / SIGN UP
              </button>
            )}
          </div>
          ...
          <div className="nav-mobile" style={{display:"none",alignItems:"center",gap:8}}>
            {user&&(
              <div onClick={()=>setModal(true)} style={{display:"flex",alignItems:"center",gap:5,background:th.surface,border:`1px solid ${credits<=3&&credits>0?"rgba(239,68,68,0.4)":th.border}`,borderRadius:20,padding:"5px 10px",cursor:"pointer"}}>
                <span style={{fontSize:12}}>⚡</span>
                <span style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:12,color:credits<=3&&credits>0?"#ef4444":"#00d4aa"}}>{credits}</span>
              </div>
            )}
            <button onClick={()=>setMobileMenu(!mobileMenu)} style={{background:th.surface,border:`1px solid ${th.border}`,borderRadius:10,padding:"7px 10px",cursor:"pointer",display:"flex",flexDirection:"column",gap:3,justifyContent:"center"}}>
              <div style={{width:16,height:2,background:th.text,borderRadius:2,transition:"all 0.2s",transform:mobileMenu?"rotate(45deg) translate(4px,4px)":"none"}}/>
              <div style={{width:16,height:2,background:th.text,borderRadius:2,opacity:mobileMenu?0:1,transition:"all 0.2s"}}/>
              <div style={{width:16,height:2,background:th.text,borderRadius:2,transition:"all 0.2s",transform:mobileMenu?"rotate(-45deg) translate(4px,-4px)":"none"}}/>
            </button>
          </div>
        </div>
        ...
        {mobileMenu&&(
          <div className="nav-mobile" style={{display:"flex",flexDirection:"column",gap:8,marginTop:14,paddingTop:14,borderTop:`1px solid ${th.border}`,maxWidth:900,marginLeft:"auto",marginRight:"auto",animation:"fadeUp 0.2s ease"}}>
            <button onClick={()=>{toggleTheme();}} style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:th.surface,border:`1px solid ${th.border}`,borderRadius:10,padding:"10px 14px",cursor:"pointer",color:th.text,fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:12}}>
              <span>Theme</span><span>{theme==="dark"?"☀️ Light":"🌙 Dark"}</span>
            </button>
            {user?(
              <>
                <button onClick={()=>{setModal(true);setMobileMenu(false);}} style={{background:"linear-gradient(135deg,#00d4aa,#0ea5e9)",border:"none",borderRadius:10,padding:"12px",color:"#030712",fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:12,letterSpacing:1,cursor:"pointer"}}>
                  {t.buyCredits}
                </button>
                <div style={{display:"flex",alignItems:"center",gap:10,background:th.surface,border:`1px solid ${th.border}`,borderRadius:10,padding:"10px 14px"}}>
                  <div style={{width:28,height:28,borderRadius:"50%",background:"linear-gradient(135deg,#00d4aa,#0ea5e9)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:900,color:"#030712",flexShrink:0}}>
                    {(user.email||"U")[0].toUpperCase()}
                  </div>
                  <span style={{fontSize:12,color:th.text,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user.email}</span>
                  <button onClick={()=>{signOut();setMobileMenu(false);}} style={{background:"none",border:`1px solid #ef444455`,borderRadius:8,padding:"6px 10px",color:"#ef4444",fontSize:11,cursor:"pointer",fontFamily:"'Syne',sans-serif",fontWeight:700}}>LOGOUT</button>
                </div>
              </>
            ):(
              <button onClick={()=>{setShowAuth(true);setMobileMenu(false);}} style={{background:"linear-gradient(135deg,#00d4aa,#0ea5e9)",border:"none",borderRadius:10,padding:"12px",color:"#030712",fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:12,letterSpacing:1,cursor:"pointer"}}>
                LOGIN / SIGN UP
              </button>
            )}
          </div>
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:900,margin:"0 auto",padding:"0 16px 80px"}}>
        ...
        {/* HOME */}
        {!mod&&(
          <div className="home-hero" style={{paddingTop:44,paddingBottom:36,animation:"fadeUp 0.5s ease"}}>
            {/* 2-column hero grid: left=text, right=stats */}
            <div className="hero-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:32,marginBottom:32,alignItems:"center"}}>
              {/* LEFT */}
              <div>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
                  <div style={{width:8,height:8,borderRadius:"50%",background:"#00d4aa",boxShadow:"0 0 12px #00d4aa",flexShrink:0}}/>
                  <span className="tagline-text" style={{fontFamily:"'Syne',sans-serif",fontSize:12,fontWeight:800,letterSpacing:3,color:"#00d4aa",lineHeight:1.4}}>{t.tagline}</span>
                </div>
                <h1 className="home-h1" style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(28px,5vw,56px)",fontWeight:900,lineHeight:1.05,letterSpacing:-1,marginBottom:14,color:th.text}}>
                  {t.h1a}<br/>
                  <span style={{WebkitTextStroke:`1px ${th.border}`,color:"transparent",display:"inline-block",maxWidth:"100%"}}>{t.h1b}</span>
                </h1>
                <p style={{color:th.muted,fontSize:14,lineHeight:1.7,fontWeight:300,marginBottom:0}}>{t.desc}</p>
              </div>
              {/* RIGHT — Stats */}
              <div className="hero-stats" style={{display:"flex",flexDirection:"column",gap:10}}>
                {[
                  {label:"Gig Ranking Factors", val:"11",   color:"#00d4aa"},
                  {label:"Analysis Modules",  val:"6",    color:"#0ea5e9"},
                  {label:"Credits / Analysis",val:"1–3",  color:"#f59e0b"},
                  {label:"Algorithm Version", val:"2026", color:"#7c3aed"},
                ].map(({label,val,color})=>(
                  <div key={label} style={{background:th.surface,border:`1px solid ${th.border}`,borderRadius:12,padding:"13px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
                    <span style={{fontSize:12,color:th.muted,fontWeight:500}}>{label}</span>
                    <span style={{fontFamily:"'Syne',sans-serif",fontSize:20,fontWeight:900,color,flexShrink:0}}>{val}</span>
                  </div>
                ))}
              </div>
            </div>
            {/* Algo factors — collapsible */}
            <AlgoFactorsCollapse th={th} t={t}/>
            {/* No credit warning */}
            {credits===0&&(
          ...
          <div style={{paddingTop:32,animation:"fadeUp 0.4s ease"}}>
            ...
            {!loading&&result&&ResC&&(
              <div style={{animation:"fadeUp 0.5s ease"}}>
                ...
                <div className="two-col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:16}}>
      ...
      {modal&&<PricingModal onClose={()=>setModal(false)} onAdded={updC} t={t} th={th} userId={user?.id}/>}
      {showAuth&&<AuthModal onClose={()=>setShowAuth(false)} onAuth={u=>{setUser(u);setShowAuth(false);}} th={th}/>}
      {authLoading&&(
        <div style={{position:"fixed",inset:0,zIndex:9999,background:th.bg,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16}}>
          <div style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:22,color:th.text}}>Gig<span style={{color:"#00d4aa"}}>Ranking</span></div>
          <div style={{width:36,height:36,border:`3px solid ${th.border}`,borderTopColor:"#00d4aa",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>
        </div>
      )}
    </div>
  );
