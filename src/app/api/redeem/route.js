...
export async function POST(req) {
  ...
  try {
    ...
    const row2 = idx + 2;
    await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/Sheet1!C${row2}:E${row2}?valueInputOption=RAW`, { method: "PUT", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ values: [["USED", userId, new Date().toISOString()]] }) });
