...
### v3.7 — Mobile overflow fixes
**Date:** Current

- **Fixed:** Content was getting cut off on the left side on mobile — main container now has proper left/right padding on small screens.
- **Fixed:** "11 Factors" pill badge was overflowing outside the button boundary on mobile. Button now has `overflow:hidden` and the pill uses `whiteSpace:nowrap` + `flexShrink:0` to stay inside.
- **Fixed:** Tagline ("FIVERR GROWTH INTELLIGENCE — 2026 ALGORITHM") was wrapping awkwardly. Changed layout so dot and text stay together cleanly; font-size and letter-spacing reduce further at 480px.

---

### v3.6 — Grid layout fix for algo breakdown + H1 overflow fix
**Date:** Current

- **Fixed:** Algorithm breakdown button text was wrapping on mobile (e.g. "2026 Algorithm Breakdown (11 Factors)" was breaking to two lines inside the button). Now separated into button text + a small "11 Factors" pill badge — stays on one line at all sizes.
- **Fixed:** Toggle button now spans full width so it aligns with the grid below it cleanly.
- **Fixed:** Factor cards inside the collapsed breakdown now use `.algo-grid` (2 columns, collapses to 1 on phones under 480px) and look cleaner — colored dot instead of plain text bullet, glowing box-shadow on dot, better alignment.
- **Fixed:** "ANALYZER" outline text was overflowing its container on narrow screens. H1 now uses `clamp(28px,8vw,56px)` with the outline span set to `display:inline-block;maxWidth:100%` so it scales cleanly without overflow.
- **Added:** `.algo-grid` CSS class with responsive breakpoint.
